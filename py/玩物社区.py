import re
import json
import urllib.parse
import requests
from base64 import b64encode

try:
    from Crypto.Cipher import AES
    from Crypto.Util.Padding import unpad
    HAS_CRYPTO = True
except ImportError:
    HAS_CRYPTO = False
#发布页https://wanwuu.com/contact
#https://wanwuu.pages.dev
#https://wanwuu.github.io
class Spider:
    def __init__(self):
        self.name = '玩物社区'
        self.host = 'https://nzc.obifwihr.cc'
        self.default_pic = 'https://via.placeholder.com/400x225?text=Video'
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Linux; Android 13; SM-S901U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Mobile Safari/537.36',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
            'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
            'Referer': self.host,
        }
        self.classes = []
        category_str = "当前最热$popular#最近更新$new#直播回放$zhibo-huifang#国产sm$guochan-sm#日韩sm$rihan-sm#欧美sm$oumei-sm#动漫sm$dongman-sm#调教av$tiaojiao-av"
        for item in category_str.split('#'):
            if '$' in item:
                name, path = item.split('$')
                self.classes.append({"type_name": name, "type_id": path})

    def _needs_decrypt(self, pic_url):
        domains = ['cshsst.cn', 'gzyfhd.cn', 'qbpfpu.cn', 'iqdeyg.cn', 'akclqx.cn', 'sutxox.cn', 'mxlycf.cn', 'cshsst.cn', 'cshsst.cn', 'cshsst.cn', 'cshsst.cn', 'nuxmfp.cn']
        return any(d in pic_url for d in domains)

    def _decrypt_pic(self, img_url):
        if not HAS_CRYPTO:
            return img_url 
        try:
            img_headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.7390.55 Safari/537.36',
                'sec-ch-ua': '"Not/A)Brand";v="8", "Chromium";v="141", "Google Chrome";v="141"',
                'Origin': self.host,
                'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
            }
            response = requests.get(img_url, headers=img_headers, timeout=5)
            if response.status_code != 200:
                return self.default_pic
            key = b"f5d965df75336270"
            iv = b'97b60394abc2fbe1'
            cipher = AES.new(key, AES.MODE_CBC, iv)
            pt = unpad(cipher.decrypt(response.content), AES.block_size)
            b64_code = b64encode(pt).decode()
            return f"data:image/jpeg;base64,{b64_code}"
        except Exception as e:
            return self.default_pic

    def getDependence(self): return []
    def getName(self): return self.name
    def isVideoFormat(self, url): return False
    def manualVideoCheck(self): pass
    def init(self, extend=""): pass

    def homeContent(self, filter):
        return {"class": self.classes}

    def homeVideoContent(self):
        try:
            r = requests.get(self.host, headers=self.headers, timeout=10)
            r.encoding = 'utf-8'
            videos = self._parse_videos(r.text)
            return {"list": videos}
        except Exception as e:
            return {"list": []}

    def _parse_videos(self, html):
        videos = []
        blocks = re.findall(r'<div class="video-item" href="([^"]+)">(.*?)<div class="truncate', html, re.S)
        for href, content in blocks:
            title = ""
            title_match = re.search(r'alt="([^"]+)"', content)
            if title_match:
                title = title_match.group(1).strip()
            else:
                a_match = re.search(r'<a class="my-1[^>]+>(.*?)</a>', content, re.S)
                if a_match: 
                    title = re.sub(r'<[^>]+>', '', a_match.group(1)).strip()
            if not title: continue

            pic = ""
            pic_match = re.search(r'data-src="([^"]+)"', content)
            if pic_match:
                pic = pic_match.group(1)
            else:
                src_match = re.search(r'src="([^"]+)"', content)
                if src_match and "poster_loading" not in src_match.group(1):
                    pic = src_match.group(1)

            pic = self._abs(pic)
            if self._needs_decrypt(pic):
                pic = self._decrypt_pic(pic)

            remark = ""
            dur_match = re.search(r'opacity-50[^>]*>\s*([\d:]+)\s*<', content)
            if dur_match:
                remark = dur_match.group(1)

            videos.append({
                "vod_id": href,
                "vod_name": title,
                "vod_pic": pic,
                "vod_remarks": remark
            })
        return videos

    def categoryContent(self, tid, pg, filter, extend):
        try:
            pg = int(pg)
            prefix = "/videos/" if not tid.startswith("videos/") else "/"
            url = f"{self.host}{prefix}{tid}/page/{pg}/" if pg > 1 else f"{self.host}{prefix}{tid}/"
            url = url.replace('//videos', '/videos')
            
            r = requests.get(url, headers=self.headers, timeout=10)
            r.encoding = 'utf-8'
            return self._page(self._parse_videos(r.text), pg)
        except Exception as e:
            return self._page([], pg)

    def searchContent(self, key, quick, pg='1'):
        try:
            pg = int(pg)
            wd = urllib.parse.quote(key)
            url = f"{self.host}/videos/search/{wd}/page/{pg}/" if pg > 1 else f"{self.host}/videos/search/{wd}/"
            r = requests.get(url, headers=self.headers, timeout=10)
            r.encoding = 'utf-8'
            return self._page(self._parse_videos(r.text), pg)
        except Exception as e:
            return self._page([], pg)

    def detailContent(self, array):
        vid = array[0]
        if not vid.startswith('http'):
            vid = self._abs(vid)
            
        try:
            r = requests.get(vid, headers=self.headers, timeout=10)
            r.encoding = 'utf-8'
            html = r.text

            title = ""
            pic = ""
            desc = ""
            play_url = vid
            vod_class = ""
            vod_actor = ""
            vod_duration = ""

            detail_match = re.search(r'const _detail_ = (\{.*?\})', html, re.S)
            if detail_match:
                try:
                    detail_json = json.loads(detail_match.group(1))
                    title = detail_json.get("title", "")
                    pic = detail_json.get("poster", "")
                    pic = pic.replace('\\/', '/')
                    video_id = detail_json.get("id", "")
                    if video_id:
                        play_url = f"{self.host}/videos/embed?id={video_id}"
                    vod_duration = detail_json.get("video_duration", "")
                    tags = detail_json.get("tag", [])
                    if isinstance(tags, list):
                        vod_class = ",".join(tags)
                    vod_actor = detail_json.get("user_name", "")
                except:
                    pass

            if not title:
                h1_match = re.search(r'<h1[^>]*dx-title[^>]*>(.*?)</h1>', html)
                if h1_match:
                    title = h1_match.group(1).strip()
                else:
                    title_match = re.search(r'<meta property="og:title" content="(.*?)"', html)
                    if title_match: title = title_match.group(1).strip()

            if not pic:
                pic_match = re.search(r'<meta property="og:image" content="(.*?)"', html)
                if pic_match:
                    pic = pic_match.group(1)

            if not desc:
                desc_match = re.search(r'<meta property="og:description" content="(.*?)"', html)
                if desc_match: desc = desc_match.group(1).strip()

            pic = self._abs(pic)
            if self._needs_decrypt(pic): 
                pic = self._decrypt_pic(pic)

            if "videos/embed" not in play_url:
                vid_match = re.search(r'data-video_id="(\d+)"', html)
                if not vid_match:
                    vid_match = re.search(r'videos/embed\?id=(\d+)', html)
                if vid_match:
                    video_id = vid_match.group(1)
                    play_url = f"{self.host}/videos/embed?id={video_id}"

            vod = {
                "vod_id": vid,
                "vod_name": title,
                "vod_pic": pic,
                "type_name": "视频",
                "vod_class": vod_class,
                "vod_actor": vod_actor,
                "vod_remarks": vod_duration,
                "vod_content": desc,
                "vod_play_from": "玩物社区",
                "vod_play_url": f"点击播放${play_url}"
            }
            return {"list": [vod]}
        except Exception as e:
            return {"list": []}

    def playerContent(self, flag, id, vipFlags):
        return {
            "parse": 1,
            "playUrl": "",
            "url": id, 
            "header": {
                "User-Agent": self.headers['User-Agent'],
                "Referer": self.host
            }
        }

    def _abs(self, url):
        if not url: return ""
        if url.startswith("data:image"): return url
        if url.startswith('http'): return url
        if url.startswith('//'): return 'https:' + url
        return urllib.parse.urljoin(self.host, url)

    def _page(self, videos, pg):
        return {
            "list": videos, 
            "page": int(pg), 
            "pagecount": 9999, 
            "limit": 24, 
            "total": 999999
        }
    
    def localProxy(self, param):
        return []
