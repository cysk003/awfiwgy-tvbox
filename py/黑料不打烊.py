...
class Spider(BaseSpider):
    ...
    def get_working_host(self):
        dynamic_urls = [
            'https://angle.jvhpfxe.com',
            'https://www.rhqaqxud.cc',
            'https://abdy14.com', 
            'https://www.njqynwy.cc'
        ]
    ...
    def detailContent(self, ids):
        try:
            ...
            if not vod_content:
                vod_content = data('h1').text() or '黑料不打烊'

            return {'list': [{'vod_play_from': '黑料不打烊', 'vod_play_url': play_url, 'vod_content': vod_content}]}
        except:
            return {'list': [{'vod_play_from': '黑料不打烊', 'vod_play_url': '获取失败'}]}
